...
import { ChevronLeft, ChevronRight, ImagePlus, Pencil, Plus, Search, Star, Trash2, X } from "lucide-react";
...
export default function PromocionesTiendas() {
  ...
  const editar = (promo: Promocion) => {
    ...
    setForm({
      ...
      publicada: Boolean(promo.publicada),
  ...
  const guardar = async (event: FormEvent) => {
    ...
    if (!imagen && !form.imagen_url && !editando?.imagen) {
      Swal.fire("Imagen requerida", "Carga una imagen o proporciona una URL de imagen.", "warning");
      return;
    }
    ...
    try {
      ...
      await cargar();
      Swal.fire("Guardada", "La promoción fue guardada correctamente.", "success");
    } catch (error: any) {
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? <tr><td colSpan={6} className="h-40 text-center font-semibold text-gray-500">Cargando promociones...</td></tr> : promociones.length === 0 ? <tr><td colSpan={6} className="h-40 text-center font-semibold text-gray-500">No hay promociones registradas</td></tr> : promociones.map((promo) => (
                  <tr key={promo.id} className="hover:bg-red-50/30">
                    ...
                    <td className="px-5 py-4 text-center">{promo.destacada ? <Star className="mx-auto fill-amber-400 text-amber-500" size={18} /> : <span className="text-gray-300">-</span>}</td>
...
function ModalPromocion({ form, setForm, preview, setPreview, setImagen, saving, editando, onClose, onSubmit }: any) {
  return <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-sm" onMouseDown={onClose}><section className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white shadow-2xl" onMouseDown={(e) => e.stopPropagation()}><header className="flex items-center justify-between border-b border-gray-200 px-6 py-5"><div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-red-950">Contenido promocional</p><h2 className="mt-1 text-xl font-black">{editando ? "Editar promoción" : "Nueva promoción"}</h2></div><button title="Cerrar" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-xl border border-gray-200"><X size={18} /></button></header><form onSubmit={onSubmit} className="grid gap-5 p-6 md:grid-cols-2">
    <Campo label="Tienda"><input required value={form.tienda} onChange={(e) => setForm({ ...form, tienda: e.target.value })} className={inputClase} /></Campo><Campo label="Título"><input required maxLength={180} value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} className={inputClase} /></Campo>
    <Campo label="Categoría"><input value={form.categoria} onChange={(e) => setForm({ ...form, categoria: e.target.value })} className={inputClase} placeholder="Tecnología, hogar, moda..." /></Campo><Campo label="Evento"><input value={form.evento} onChange={(e) => setForm({ ...form, evento: e.target.value })} className={inputClase} placeholder="Black Friday, Mother's Day..." /></Campo>
    ...
    <Campo label="Enlace de la oferta"><input required type="url" value={form.url_destino} onChange={(e) => setForm({ ...form, url_destino: e.target.value })} className={inputClase} placeholder="https://..." /></Campo><Campo label="URL externa de imagen (opcional)"><input type="url" value={form.imagen_url} onChange={(e) => { setForm({ ...form, imagen_url: e.target.value }); if (!preview?.startsWith("blob:")) setPreview(e.target.value || null); }} className={inputClase} placeholder="https://..." /></Campo>
    <Campo label="Fecha de inicio"><input required type="datetime-local" value={form.fecha_inicio} onChange={(e) => setForm({ ...form, fecha_inicio: e.target.value })} className={inputClase} /></Campo><Campo label="Fecha de finalización"><input required type="datetime-local" value={form.fecha_fin} onChange={(e) => setForm({ ...form, fecha_fin: e.target.value })} className={inputClase} /></Campo>
    <Campo label="Orden de aparición"><input type="number" min="0" value={form.orden} onChange={(e) => setForm({ ...form, orden: e.target.value })} className={inputClase} /></Campo><label><span className="text-sm font-bold text-gray-700">Imagen en Azure Storage</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => { const file = e.target.files?.[0] || null; setImagen(file); if (file) setPreview(URL.createObjectURL(file)); }} className="mt-2 block w-full text-sm font-semibold text-gray-600 file:mr-4 file:rounded-xl file:border-0 file:bg-red-50 file:px-4 file:py-3 file:font-bold file:text-red-950" /></label>
    {preview && <div className="md:col-span-2"><img src={preview} alt="Vista previa" className="h-48 w-full rounded-xl border border-gray-200 object-cover" /></div>}
    <div className="flex flex-wrap gap-5 md:col-span-2"><label className="flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={form.publicada} onChange={(e) => setForm({ ...form, publicada: e.target.checked })} className="h-5 w-5 accent-red-950" /> Publicar según vigencia</label><label className="flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={form.destacada} onChange={(e) => setForm({ ...form, destacada: e.target.checked })} className="h-5 w-5 accent-red-950" /> Promoción destacada</label></div>
    <div className="flex justify-end gap-3 border-t border-gray-200 pt-5 md:col-span-2"><button type="button" onClick={onClose} className="rounded-xl border border-gray-200 px-5 py-3 text-sm font-bold">Cancelar</button><button disabled={saving} className="rounded-xl bg-gradient-to-r from-red-950 to-red-900 px-7 py-3 text-sm font-black text-white disabled:opacity-60">{saving ? "Guardando..." : "Guardar promoción"}</button></div>
  </form></section></div>;
}
