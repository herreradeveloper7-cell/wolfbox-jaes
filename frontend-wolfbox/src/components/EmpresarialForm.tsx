...
export default function EmpresarialForm({ tipoCliente }: Props) {
  ...
  const cityOptions =
    ...
    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
      ...
      <div>
        <label className="block font-medium mb-1">NIT o Número de identificación *</label>
        <input 
        type="text" 
      ...
      <div>
        ...
        className={`w-full p-2 border rounded ${
          !errores.emailCoincide ? 'border-red-500' : ''
        }`} 
        required 
        />
        {!errores.emailCoincide && (
          <p className="text-red-500 text-sm mt-1">Los correos no coinciden</p>
        )}
      </div>
      ...
      <div>
        ...
        <input 
          ...
          className={`w-full p-2 border rounded ${!errores.contrasenaCoincide ? 'border-red-500' : ''}`}
          required
        />
